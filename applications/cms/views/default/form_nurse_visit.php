	...
	<?php foreach ($nurse_visit['forms'] as $form): $id = 'id_' . $form->tbl; ?>
		<div class="panel panel-default">
			...
			<div id="<?php echo $form->tbl . '_' . $form->$id ?>" class="panel-collapse collapse">
	            <div class="panel-body" form-tbl="<?php echo $form->name ?>" form-id="<?php echo $form->$id ?>" id="nurse_visit_data_<?php echo $form->$id ?>_<?php echo $form->tbl ?>"></div>
	        </div>
		</div>
	<?php endforeach ?>
	</div>
...
$(function(){
	...
	$(document).on('click', 'a[id^="create-new-note-form_"]', function() {
        ...
        ajaxPatient(form_to_create, 'result_nurse_visit');
        return false;
    });
});
</script>